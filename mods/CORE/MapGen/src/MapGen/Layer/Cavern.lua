local mathMin,  mathMax,  mathAbs
	= math.min, math.max, math.abs

---@class MapGen.Layer.Cavern
---@field name            string
---@field minY            number
---@field maxY            number
---@field smoothDistance  number
---@field getNoise        fun():ValueNoise
---@field initNoise       fun()
---@field getGroups       fun():table<string, number>
local Cavern = {}

---@param name            string
---@param minY            number
---@param maxY            number
---@param smoothDistance  number
---@param noiseParams     NoiseParams
---@param groups?         table<string, number>
---@return                MapGen.Layer.Cavern
function Cavern:new(name, minY, maxY, smoothDistance, noiseParams, groups)
	assert(maxY > minY, 'maxY there should be more minY')
	assert(smoothDistance < mathAbs(maxY - minY)/2, 'The smoothing distance cannot be greater than half the distance between minY and maxY')

	---@type NoiseParams
	local _noiseParams = noiseParams

	---@diagnostic disable-next-line: missing-fields
	---@type ValueNoise
	local _noise = {}-- note: must be nil until luanti mapgen objects are loaded.

	local _groups

	if groups == nil then
		_groups = {}
	else
		_groups = groups
	end

	---@type MapGen.Layer.Cavern
	local instance = setmetatable({
		name = name,
		minY = minY,
		maxY = maxY,
		smoothDistance = smoothDistance,
	}, {__index = self})

	function instance:getNoise()
		return _noise
	end

	function instance:initNoise()
		if not table.is_empty(_noise) then
			minetest.log('warning', 'Cavern noise has already been initialized. Re-initialization is not recommended.')
		end

		_noise = core.get_value_noise(noiseParams)
	end

	function instance:getGroups()
		return table.copy_with_metatables(_groups)
	end

	return instance
end

---@param y               number
---@param minY            number
---@param maxY            number
---@param smoothDistance  number
---@return                number
local function calcCavernWeight(y, smoothDistance, minY, maxY)
	if y > maxY or y < minY then
		return 0
	end

	local maxDist = maxY - y
	local minDist = y - minY

	return mathMax(0, mathMin(maxDist / smoothDistance, minDist / smoothDistance, 1))
end

---@param x          number
---@param y          number
---@param z          number
---@param threshold  number?
---@return           boolean
function Cavern:isCavern(x, y, z, threshold)
	if threshold == nil then
		threshold = 0
	end

	local noise = self:getNoise()
	local weight = calcCavernWeight(y, self.smoothDistance, self.minY, self.maxY)

	if y == -15 then
		print('is', weight)
	end

	return noise:get_3d({x = x, y = y, z = z}) * weight > threshold
end

return Cavern