...
local mathMin, mathMax, mathHuge, mathAbs
	= math.min, math.max, math.huge, math.abs
...
function Polyhedron:new(vertices, faces)
	...
	for i, face in ipairs(faces) do
		instance.faceData[i] = {
			plane = instance:computePlane(face),
			normal = instance:computeFaceNormal(face),
			edges = instance:computeFaceEdges(face)
		}
	end
...
function Polyhedron:computeFaceEdges(face)
	local edges = {}
	for i = 1, #face do
		local j = (i % #face) + 1
		edges[i] = {face[i], face[j]}
	end
	return edges
end
...
function Polyhedron:computeFaceNormal(face)
	...
	for i = 1, #face do
		...
		local next_vert = self.vertices[face[j]]

		-- Newell's method components
		normal.x = normal.x + (current.y - next_vert.y) * (current.z + next_vert.z)
		normal.y = normal.y + (current.z - next_vert.z) * (current.x + next_vert.x)
		normal.z = normal.z + (current.x - next_vert.x) * (current.y + next_vert.y)
	end

	-- Check for zero normal (degenerate face)
	local magnitude = vector.length(normal)
	if magnitude == 0 then
		return vector.zero()  -- Or handle degenerate case appropriately
	end
...
function Polyhedron:containsPoint(point)
	...
	local bb = self.boundingBox
	if point.x < bb.min.x or point.x > bb.max.x or
	   point.y < bb.min.y or point.y > bb.max.y or
	   point.z < bb.min.z or point.z > bb.max.z then
		return false
	end
	
	-- Then do ray casting
	local rayDir = vector.new(1, 0, 0)
